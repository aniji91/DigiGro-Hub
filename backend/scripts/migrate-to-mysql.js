      ...
      await connection.query(
        `INSERT INTO projects
         (id, name, client_name, client_id, description, status, start_date, end_date,
          ...
          production_details, external_crm_integrations, timeline_tasks, is_hidden, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          item.id,
          item.name,
          ...
          JSON.stringify(item.timelineTasks || []),
          item.isHidden ? 1 : 0,
          item.createdAt || null,
        ]
      );
      ...
      await connection.query(
        `INSERT INTO project_updates
         (id, project_id, project_name, update_date, update_type, content, task_status, due_at,
          overdue_note, assigned_employee_id, assigned_employee_name, is_hidden, author_id, author_name, author_role, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          item.id,
          item.projectId,
